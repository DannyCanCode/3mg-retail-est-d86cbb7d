  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* Accessories & Additional Items from Job Worksheet */}
        {jobWorksheet && (() => {
          ...
          if (jobWorksheet.accessories?.skylight) {
            if (jobWorksheet.accessories.skylight.count_2x2 > 0) {
              accessoryItems.push({ 
                name: '2x2 Skylights', 
                quantity: jobWorksheet.accessories.skylight.count_2x2 
              });
            }
            if (jobWorksheet.accessories.skylight.count_2x4 > 0) {
              accessoryItems.push({ 
                name: '2x4 Skylights', 
                quantity: jobWorksheet.accessories.skylight.count_2x4 
              });
            }
          }
          
          // Solar Attic Fans
          if (jobWorksheet.accessories?.solar_attic_fans) {
            if (jobWorksheet.accessories.solar_attic_fans.kennedy_35w > 0) {
              accessoryItems.push({ 
                name: 'Kennedy 35W Solar Attic Fans', 
                quantity: jobWorksheet.accessories.solar_attic_fans.kennedy_35w 
              });
            }
            if (jobWorksheet.accessories.solar_attic_fans.attic_breeze_45w > 0) {
              accessoryItems.push({ 
                name: 'Attic Breeze 45W Solar Attic Fans', 
                quantity: jobWorksheet.accessories.solar_attic_fans.attic_breeze_45w 
              });
            }
          }
          ...
          if (jobWorksheet.gutters?.leaf_guards?.install && jobWorksheet.gutters.leaf_guards.linear_feet > 0) {
            accessoryItems.push({ 
              ...
              quantity: `${jobWorksheet.gutters.leaf_guards.linear_feet} LF` 
            });
          }
          
          // Gutters
          if (jobWorksheet.gutters?.keep_or_new === 'new' && jobWorksheet.gutters.gutter_lf > 0) {
            const gutterSize = jobWorksheet.gutters.gutter_size === '6_inch' ? '6"' : '7"';
            accessoryItems.push({ 
              name: `New ${gutterSize} Gutters (${jobWorksheet.gutters.gutter_color || 'White'})`, 
              quantity: `${jobWorksheet.gutters.gutter_lf} LF` 
            });
          }
          
          // Detach & Reset Gutters
          if (jobWorksheet.gutters?.detach_reset_gutters && jobWorksheet.gutters.detach_reset_gutter_lf > 0) {
            accessoryItems.push({ 
              name: 'Detach & Reset Gutters', 
              quantity: `${jobWorksheet.gutters.detach_reset_gutter_lf} LF` 
            });
          }
          ...
          if (jobWorksheet.ventilation) {
            // Ridge Vents
            if (jobWorksheet.ventilation.ridge_vents_lf > 0) {
              accessoryItems.push({ 
                name: 'Ridge Vents', 
                quantity: `${jobWorksheet.ventilation.ridge_vents_lf} LF` 
              });
            }
            
            // Off Ridge Vents
            if (jobWorksheet.ventilation.off_ridge_vents) {
              if (jobWorksheet.ventilation.off_ridge_vents['2_ft'] > 0) {
                accessoryItems.push({ 
                  name: '2 ft Off Ridge Vents', 
                  quantity: jobWorksheet.ventilation.off_ridge_vents['2_ft'] 
                });
              }
              if (jobWorksheet.ventilation.off_ridge_vents['4_ft'] > 0) {
                accessoryItems.push({ 
                  name: '4 ft Off Ridge Vents', 
                  quantity: jobWorksheet.ventilation.off_ridge_vents['4_ft'] 
                });
              }
            }
            
            // Goosenecks
            if (jobWorksheet.ventilation.goosenecks) {
              const gooseneckSizes = ['4_inch', '6_inch', '10_inch', '12_inch'];
              gooseneckSizes.forEach(size => {
                if (jobWorksheet.ventilation.goosenecks[size] > 0) {
                  const displaySize = size.replace('_', ' ').replace('inch', '"');
                  accessoryItems.push({ 
                    name: `${displaySize} Goosenecks (${jobWorksheet.ventilation.goosenecks.color || 'Black'})`, 
                    quantity: jobWorksheet.ventilation.goosenecks[size] 
                  });
                }
              });
            }
            ...
            // Boots
            if (jobWorksheet.ventilation.boots) {
              const bootSizes = ['1_5_inch', '2_inch', '3_inch', '4_inch'];
              bootSizes.forEach(size => {
                if (jobWorksheet.ventilation.boots[size] > 0) {
                  const displaySize = size.replace(/_/g, '.').replace('inch', '"');
                  accessoryItems.push({ 
                    name: `${displaySize} Boots (${jobWorksheet.ventilation.boots.color || 'Black'})`, 
                    quantity: jobWorksheet.ventilation.boots[size] 
                  });
                }
              });
            }
