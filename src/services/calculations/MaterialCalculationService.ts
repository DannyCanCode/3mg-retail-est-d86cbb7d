...
/**
 * Service responsible for all material quantity calculations
 */
export class MaterialCalculationService {
  /**
   * Calculate the quantity of a material needed based on measurements
   * 
   * @param material - The material to calculate quantity for
   * @param measurements - Roof measurements
   * @param wasteFactor - Waste factor as a decimal (default 0.1 = 10%)
   * @returns The calculated quantity
   */
  public static calculateMaterialQuantity(
    material: Material,
    ...
    // Use appropriate calculation method based on material category
    switch (material.category) {
      case MaterialCategory.SHINGLES:
        return this.calculateShingleQuantity(material, measurements, wasteFactor);
      case MaterialCategory.UNDERLAYMENTS:
        return this.calculateUnderlaymentQuantity(material, measurements, wasteFactor);
      case MaterialCategory.LOW_SLOPE:
        return this.calculateLowSlopeQuantity(material, measurements, wasteFactor);
      case MaterialCategory.ACCESSORIES:
        return this.calculateAccessoryQuantity(material, measurements, wasteFactor);
      default:
        // Default to 0 for unknown categories
        console.log(`[CalcQuantity] Unknown category: ${material.category}, returning 0.`);
        return 0;
    }
  }

  /**
   * Calculate the quantity of shingle materials needed
   */
  private static calculateShingleQuantity(
    material: Material,
    measurements: MeasurementValues,
    wasteFactor: number
  ): number {
    ...
    if (material.id.includes('seal-a-ridge') || material.id.includes('hip-ridge')) {
      ...
      const lfPerBundle = this.extractCoverageValue(material.coverageRule.description) || 25; // Default 25 if extraction fails
    ...
    // --- Starter Shingles --- 
      ...
      const lfPerBundle = this.extractCoverageValue(material.coverageRule.description) || 120; // Default 120
  ...
  /**
   * Calculate the quantity of underlayment materials needed
   */
  private static calculateUnderlaymentQuantity(
    material: Material,
    measurements: MeasurementValues,
    wasteFactor: number
  ): number {
    ...
    if (calculationArea <= 0) {
      return 0; // Ensure quantity is 0 if calculation area is 0
    }

    const totalSquares = calculationArea / 100;
    // Extract coverage per roll (e.g., 4.5 for Rhino, 10 for Feltbuster, 1.5 for Ice&Water)
    const squaresPerRoll = material.coverageAmount || this.extractCoverageValue(material.coverageRule.description) || 4; 
    ...
    const quantity = Math.ceil(finalSquares / squaresPerRoll);
  ...
  /**
   * Calculate the quantity of low slope materials needed
   */
  private static calculateLowSlopeQuantity(
    material: Material,
    measurements: MeasurementValues,
    wasteFactor: number
  ): number {
  ...
  /**
   * Calculate the quantity of accessory materials needed
   */
  private static calculateAccessoryQuantity(
    material: Material,
    measurements: MeasurementValues,
    wasteFactor: number
  ): number {
   ...
   */
  private static extractCoverageValue(description: string): number {
  ...
  /**
   * Calculate the total cost for a material based on quantity and price
   */
  public static calculateMaterialTotal(quantity: number, price: number): number {
   ...
   */
  public static groupMaterialsByCategory(materials: Material[]): Record<string, Material[]> {
