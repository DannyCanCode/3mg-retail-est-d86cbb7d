...
interface EstimateSummaryTabProps {
  ...
  onBack?: () => void;
  jobWorksheetData?: any; // Job worksheet data for accessories
}
    ...
    <>
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          {/* Accessories and Additional Items from Job Worksheet */}
          {(jobWorksheetData || estimate?.job_worksheet) && (() => {
            const worksheetData = jobWorksheetData || estimate?.job_worksheet;
            const accessoryItems = [];
            
            // Satellite Dish Removal
            if (worksheetData.ventilation?.accessories?.satellite_removal) {
              accessoryItems.push({ name: 'Satellite Dish Removal', quantity: 1 });
            }
            
            // Skylights
            if (worksheetData.accessories?.skylight) {
              if (worksheetData.accessories.skylight.count_2x2 > 0) {
                accessoryItems.push({ 
                  name: '2x2 Skylights', 
                  quantity: worksheetData.accessories.skylight.count_2x2 
                });
              }
              if (worksheetData.accessories.skylight.count_2x4 > 0) {
                accessoryItems.push({ 
                  name: '2x4 Skylights', 
                  quantity: worksheetData.accessories.skylight.count_2x4 
                });
              }
            }
            
            // Solar Attic Fans
            if (worksheetData.accessories?.solar_attic_fans) {
              if (worksheetData.accessories.solar_attic_fans.kennedy_35w > 0) {
                accessoryItems.push({ 
                  name: 'Kennedy 35W Solar Attic Fans', 
                  quantity: worksheetData.accessories.solar_attic_fans.kennedy_35w 
                });
              }
              if (worksheetData.accessories.solar_attic_fans.attic_breeze_45w > 0) {
                accessoryItems.push({ 
                  name: 'Attic Breeze 45W Solar Attic Fans', 
                  quantity: worksheetData.accessories.solar_attic_fans.attic_breeze_45w 
                });
              }
            }
            
            // Leaf Guards
            if (worksheetData.gutters?.leaf_guards?.install && worksheetData.gutters.leaf_guards.linear_feet > 0) {
              accessoryItems.push({ 
                name: 'Gutter Leaf Guards', 
                quantity: `${worksheetData.gutters.leaf_guards.linear_feet} LF` 
              });
            }
            
            // Gutters
            if (worksheetData.gutters?.keep_or_new === 'new' && worksheetData.gutters.gutter_lf > 0) {
              const gutterSize = worksheetData.gutters.gutter_size === '6_inch' ? '6"' : '7"';
              accessoryItems.push({ 
                name: `New ${gutterSize} Gutters (${worksheetData.gutters.gutter_color || 'White'})`, 
                quantity: `${worksheetData.gutters.gutter_lf} LF` 
              });
            }
            
            // Detach & Reset Gutters
            if (worksheetData.gutters?.detach_reset_gutters && worksheetData.gutters.detach_reset_gutter_lf > 0) {
              accessoryItems.push({ 
                name: 'Detach & Reset Gutters', 
                quantity: `${worksheetData.gutters.detach_reset_gutter_lf} LF` 
              });
            }
            
            // Ventilation items
            if (worksheetData.ventilation) {
              // Ridge Vents
              if (worksheetData.ventilation.ridge_vents_lf > 0) {
                accessoryItems.push({ 
                  name: 'Ridge Vents', 
                  quantity: `${worksheetData.ventilation.ridge_vents_lf} LF` 
                });
              }
              
              // Off Ridge Vents
              if (worksheetData.ventilation.off_ridge_vents) {
                if (worksheetData.ventilation.off_ridge_vents['2_ft'] > 0) {
                  accessoryItems.push({ 
                    name: '2 ft Off Ridge Vents', 
                    quantity: worksheetData.ventilation.off_ridge_vents['2_ft'] 
                  });
                }
                if (worksheetData.ventilation.off_ridge_vents['4_ft'] > 0) {
                  accessoryItems.push({ 
                    name: '4 ft Off Ridge Vents', 
                    quantity: worksheetData.ventilation.off_ridge_vents['4_ft'] 
                  });
                }
              }
              
              // Goosenecks
              if (worksheetData.ventilation.goosenecks) {
                const gooseneckSizes = ['4_inch', '6_inch', '10_inch', '12_inch'];
                gooseneckSizes.forEach(size => {
                  if (worksheetData.ventilation.goosenecks[size] > 0) {
                    const displaySize = size.replace('_', ' ').replace('inch', '"');
                    accessoryItems.push({ 
                      name: `${displaySize} Goosenecks (${worksheetData.ventilation.goosenecks.color || 'Black'})`, 
                      quantity: worksheetData.ventilation.goosenecks[size] 
                    });
                  }
                });
              }
              ...
              // Boots
              if (worksheetData.ventilation.boots) {
                const bootSizes = ['1_5_inch', '2_inch', '3_inch', '4_inch'];
                bootSizes.forEach(size => {
                  if (worksheetData.ventilation.boots[size] > 0) {
                    const displaySize = size.replace(/_/g, '.').replace('inch', '"');
                    accessoryItems.push({ 
                      name: `${displaySize} Boots (${worksheetData.ventilation.boots.color || 'Black'})`, 
                      quantity: worksheetData.ventilation.boots[size] 
                    });
                  }
                });
              }
            ...
            return accessoryItems.length > 0 ? (
              <div className="space-y-4">
                <h3 className="text-lg font-semibold">Accessories & Additional Items</h3>
                <div className="border rounded-md">
                  <table className="w-full text-sm">
                    <thead className="bg-muted/50">
                      <tr className="border-b">
                    ...
                    <tbody>
                      {accessoryItems.map((item, index) => (
                        <tr key={index} className={index < accessoryItems.length - 1 ? "border-b" : ""}>
                          <td className="py-2 px-4">{item.name}</td>
